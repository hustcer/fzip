...
| -------- | ------ | ------------------ | ---------------------------------------------- | ----------- | ---------------------- |
| 输出文件 | `-o`   | `--output`         | 输出文件路径（非必需，支持 `-` 输出到 stdout） | 自动推断    | `tar -f`, `zip -o`     |
...
> **注意**：
>
> 1. 如果省略 `-o`，单个输入文件将自动推断输出文件名（例如被压缩为 `file.txt.gz` 或 `file.txt.zip`），目录则默认生成 `[目录名].zip`。
> 2. 当目标格式不支持多文件（如 `gzip`/`zlib`/`deflate`）但提供了多个输入文件或目录时，程序将报错中断提示（退出码 2：参数错误）。
> 3. `<INPUT>` 支持使用 `-` 表示从标准输入（stdin）读取数据。
...
> **注意**: 对于 `GZIP`, `Zlib`, `DEFLATE` 等单文件流格式，由于不包含文件表元数据，此命令可能仅显示单条记录说明，或视情况报错中断。
